...
namespace zipHMM {
  ...
  bool matches_viterbi(const s_pair &p, const std::vector<unsigned> *seq_p, const size_t idx) {
    return p.first == (*seq_p)[idx] && p.second == (*seq_p)[idx + 1];
  }

  void update_pair_n_viterbi(std::vector<size_t> &pair_n, const s_pair &pair, const size_t alphabet_size) {
   ...
   *  restriction).
   */
  void add_count_viterbi(std::vector<size_t> &pair_n, std::vector<unsigned> *seq_p, size_t position, bool &counted, size_t alphabet_size) {
    s_pair current_pair((*seq_p)[position], (*seq_p)[position+1]);
    // if it didn't match previous position, increase count
    if(!matches_viterbi(current_pair, seq_p, position-1)) {
      update_pair_n_viterbi(pair_n, current_pair, alphabet_size);
      counted = true;
  ...
  double init_apply_em_prob_viterbi(Matrix &res, const Matrix &pi, const Matrix &B, const unsigned symbol) {
  ...
  double apply_em_prob_viterbi(Matrix &res, const Matrix &A, const Matrix &B, const unsigned symbol) {
  ...
  void make_em_trans_probs_array_viterbi(double *symbol2scale, Matrix *symbol2matrix, const Matrix &A, const Matrix &B) {
    for(size_t i = 0; i < B.get_width(); ++i) {
      symbol2scale[unsigned(i)] = std::log( apply_em_prob_viterbi(symbol2matrix[unsigned(i)], A, B, unsigned(i)) );
    }
  }
